# 详细设计：Software Modeling Workbench（软件建模工作台）
...
- **不修改** `uml-vue-sdi`；本工作台为独立子项目 `software-modeling-workbench/`。

## `mv-model-codespace` 与 UML 同步契约（`uml.sync.md`）分工

- **`mv-model-codespace`**：落在 **单个 Markdown 围栏** 内的 **JSON 示意模型**（`workspaceRoot`、`modules[]`，及可选的递归 `namespaces`、Classifier、`associations` 等），由 `@mvwb/core` 的 `parseMarkdownBlocks` / `validateMvModelCodespace` 校验；适合在 Software Modeling Workbench 中与 `mv-view` 同文件编排、单块 diff。**Workbench 画布交互**（`CodespaceCanvasEditor`）：块级元数据为表单；结构在 **SVG 画布**（`CodespaceModuleSvgCanvas` + `useCanvasViewport`）中单击选中、双击打开 **`floating/*` 定义窗**（`Teleport`）；`associations[]` 仍走「高级：原始 JSON」。
