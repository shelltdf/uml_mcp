...
- **mv-model-sql 语义**（与 `@mvwb/core` 一致）：每个 `` ```mv-model-sql `` 块为 **一个 Model 组**（`tables[]` 多张子表）；围栏级 `id` 文件内唯一。解析与列/行校验见 `md-mv-core`。**mv-model-sql 画布**（`BlockCanvasPage`；`MV_MODEL_SQL_CANVAS_TITLE`）：Model 组与子表分区；子表 **标签切换**、**新增/删除子表**；当前子表 **DDL · 当前子表列定义**、**DML · 行集（JSON 单元格）** 为两块**可折叠标题栏**（整行按钮展开/折叠，默认展开），内为 DDL/DML 编辑区、**CREATE TABLE** 示意；DML 工具栏含 **WHERE 子串** 筛选与 **「显示只读视图」**（预览**全部子表**）；保存前对**所有子表**补全非空列缺键；可空列清空单元格时省略 JSON 键。
