...
import { mvViewKindStrings } from './smw-view-kind-locale';
...
export function insertModalLeadHtml(locale: AppLocale): string {
  if (locale === 'en') {
    return `Model and View are stored in Markdown as <strong>fenced code blocks</strong> (languages include <code>smw-model-sql</code> / <code>smw-model-kv</code> /
            <code>smw-model-struct</code> / <code>smw-model-codespace</code> / <code>smw-model-interface</code> / <code>smw-view</code>); the inner body may be
  ...
  return `Model 与 View 在 Markdown 中以<strong>围栏代码块</strong>落盘（围栏语言含 <code>smw-model-sql</code> / <code>smw-model-kv</code> /
            <code>smw-model-struct</code> / <code>smw-model-codespace</code> / <code>smw-model-interface</code> / <code>smw-view</code>）；块内正文可为
...
function canvasTitleForInsertCard(kind: InsertCodeBlockKind, locale: AppLocale): string {
  if (kind === 'smw-model-sql') return MV_MODEL_SQL_CANVAS_TITLE;
  if (kind === 'smw-model-kv') return MV_MODEL_KV_CANVAS_TITLE;
  if (kind === 'smw-model-struct') return MV_MODEL_STRUCT_CANVAS_TITLE;
  if (kind === 'smw-model-codespace') return shellChromeMessages[locale].canvasTitleMvModelCodespace;
  if (kind === 'smw-model-interface') return MV_MODEL_INTERFACE_CANVAS_TITLE;
...
export function insertCardTitle(kind: InsertCodeBlockKind, locale: AppLocale): string {
  const en = locale === 'en';
  if (kind === 'smw-model-sql') return en ? 'SQL model' : 'SQL数据库';
  if (kind === 'smw-model-kv') return en ? 'KV store' : 'KV数据库';
  if (kind === 'smw-model-struct') return en ? 'Structured data' : '结构化数据库';
  if (kind === 'smw-model-codespace') return en ? 'Codespace' : '代码空间';
  if (kind === 'smw-model-interface') return en ? 'Interface' : '接口图';
...
export function insertCardDesc(kind: InsertCodeBlockKind, locale: AppLocale): string {
  ...
  if (kind === 'smw-model-sql') {
    return en
      ? 'Insert ```smw-model-sql```: a **Model** group with multiple SQL-style tables (JSON); edit rows/columns in the canvas.'
      : '插入 ```smw-model-sql``` 围栏：**Model** 组，内含多张 SQL 风格表（JSON）；画布内可对子表增删改查，并编辑列与行。';
  }
  if (kind === 'smw-model-kv') {
    return en
      ? 'Insert ```smw-model-kv```: document collection (Mongo-like); edit JSON objects per row in the KV canvas.'
      : '插入 ```smw-model-kv``` 围栏：文档型集合（类比 MongoDB），每条为自由键的 JSON 对象；在 KV 数据表画布中编辑。';
  }
  if (kind === 'smw-model-struct') {
    return en
      ? 'Insert ```smw-model-struct```: recursive groups/datasets (HDF5-like); edit JSON in the struct canvas.'
      : '插入 ```smw-model-struct``` 围栏：根下递归「组 / 数据集」（类比 HDF5）；在结构化层次画布中编辑 JSON。';
  }
  if (kind === 'smw-model-codespace') {
    return en
      ? 'Insert ```smw-model-codespace```: workspace root and **modules[]** (optional namespaces, classifiers, etc.); edit in the codespace canvas.'
      : '插入 ```smw-model-codespace``` 围栏：工作区根与 **modules[]**（可选递归 **namespaces**、Classifier、bases、associations、变量/函数/宏，UML 风格示意）；在块画布的代码空间模型画布视图中编辑 JSON。';
  }
  if (kind === 'smw-model-interface') {
    return en
      ? 'Insert ```smw-model-interface```: **endpoints[]** for API sketch; edit in the interface canvas.'
      : '插入 ```smw-model-interface``` 围栏：**endpoints[]** 描述接口/端点（方法、路径、说明，文档示意）；在接口图模型画布中编辑 JSON。';
  }
  ...
  if (typeof kind === 'string' && kind.startsWith('mermaid-')) {
    return en
      ? `${base} After insert, a standard \`\`\`mermaid\`\`\` mirror fence follows the smw-view block so GitHub-style renderers can draw the diagram.`
      : `${base} 插入后在 smw-view 围栏之后附带标准 mermaid 围栏镜像段，正文与 JSON 内 payload 一致，便于 GitHub 等仅识别 Mermaid 的编辑器出图。`;
  }
  return base;
}
...
export function blockCanvasSurfaceTitle(
  kind: 'smw-model-sql' | 'smw-model-kv' | 'smw-model-struct' | 'smw-model-interface' | 'smw-map',
  locale: AppLocale,
): string {
  const en = locale === 'en';
  if (kind === 'smw-model-sql') return en ? 'Model · multi-table' : MV_MODEL_SQL_CANVAS_TITLE;
  if (kind === 'smw-model-kv') return en ? 'KV document collection' : MV_MODEL_KV_CANVAS_TITLE;
  if (kind === 'smw-model-struct') return en ? 'Structured hierarchy' : MV_MODEL_STRUCT_CANVAS_TITLE;
  if (kind === 'smw-model-interface') return en ? 'Interface diagram (sketch)' : MV_MODEL_INTERFACE_CANVAS_TITLE;
  if (kind === 'smw-map') return en ? 'Mapping rules' : MV_MAP_CANVAS_TITLE;
  return '';
}
...
export function getInsertGroups(
  ...
  return [
    { title: en ? 'Design' : '设计', kinds: ['mindmap-ui', 'ui-design'] },
    {
      ...
      kinds: ['smw-model-sql', 'smw-model-kv', 'smw-model-struct'],
    },
    { title: en ? 'Interface (Model)' : '接口模型（Model）', kinds: ['smw-model-interface'] },
    { title: en ? 'Software (Model)' : '软件模型（Model）', kinds: ['smw-model-codespace'] },
    {
