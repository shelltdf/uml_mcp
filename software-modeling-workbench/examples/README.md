# Software Modeling Workbench（软件建模工作台）— 示例与测试用工作区

本目录 **`software-modeling-workbench/examples/`** 仅供本地/人工验证 Workbench 行为，**不是** npm 包的发布内容；与仓库根 `examples/`（若有）相互独立。
...
1. 启动 Web：`npm run dev:web`（在 `software-modeling-workbench/` 根目录）。
