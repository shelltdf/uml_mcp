...
以 **Markdown 为唯一落盘格式**，用围栏块表达 **model / view / map**，支持浏览器 SPA、Electron、VS Code Webview 三壳共用 `packages/core` 与 `apps/web` 构建产物。其中 **Model 围栏**含 **`smw-model-sql`**（一个围栏内多张 SQL 风格子表）、**`smw-model-kv`**（文档型集合，MongoDB 类比）、**`smw-model-struct`**（层次组/数据集，HDF5 类比）、**`smw-model-codespace`**（工作区、`modules[]`，及可选递归 `namespaces` 与 UML 风格 Classifier/关联等示意）、**`smw-model-interface`**（接口端点列表，接口图示意）；**`smw-view`** 在契约上视为 **视图基类**，由 JSON 字段 **`kind`** 区分具体子类型；Workbench 为 **每个 kind 约定独立画布窗口**（`mermaid-*` 图源画布、脑图、**`uml-*` 独立 JSON 记录画布**、UI 设计等；`smw-model-sql` 画布内可对当前子表做只读行集预览）。详见 `packages/core` 的 `MV_VIEW_KINDS`、`MV_VIEW_KIND_METADATA` 与 `02-physical/md-mv-core/spec.md`。

## 核心与扩展边界

- 核心层（`packages/core`）只保证 `mv-*` 围栏解析、统一 JSON 契约与 `uml-*` 独立记录格式（`smw-uml/v1`）。
- `mermaid` 围栏镜像与 `@startuml` 语法均视为**扩展/插件能力**：可选、可替换，不应成为核心可用性的前置依赖。
- 对 `mermaid-*`，核心支持“有镜像则同步、无镜像也可解析”的兼容策略；上层产品可按场景决定是否启用镜像增强。

## UML 记录格式

`smw-view.kind` 为 `uml-*` 时，`payload`（非空）可为 **JSON 对象**（推荐）或 **JSON 字符串**（兼容），其对象内容需满足：
...
- 本版重点：
  - `smw-model-codespace` class 字段统一为复数：`members/methods/enums/properties`
...
| 路径 | 说明 |
|------|------|
| `packages/core` | 解析 `smw-model-sql` / `smw-model-kv` / `smw-model-struct` / `smw-model-codespace` / `smw-model-interface` / `smw-view` / `smw-map`、块回写、引用工具（纯 TS） |
...
| `packages/mcp-server` | MCP Server（stdio），提供 `smw_parse_markdown_blocks` / `smw_health` 等工具 |
...
npm test
npm run build
npm run dev:web          # 先 build @smw/core + @smw/mermaid，再 Vite 开发
...
npm run build:mcp        # build @smw/core + @smw/mcp-server
...
- `@smw/core` 不再导出 `slug`、`parseViewPayloadClassDiagram`、`buildClassDiagramViewPayload` 等 Mermaid 类图 API。
- 相关调用请改为从 `@smw/mermaid` 导入。
